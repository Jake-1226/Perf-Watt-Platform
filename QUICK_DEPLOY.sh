#!/bin/bash
# Quick Deployment Script - Run this on your VM
...
if [[ -d "perf-platform" ]]; then
    rm -rf perf-platform
fi

# IMPORTANT: Replace this with your actual repository URL
git clone https://github.com/your-org/perf-platform.git temp_repo
mv temp_repo/* .
...
cat > /opt/perf-platform/health_check.py << 'EOF'
#!/usr/bin/env python3
import subprocess
import sys
import sqlite3
...
cat > /opt/perf-platform/cli.py << 'EOF'
#!/usr/bin/env python3
import argparse
import requests
import sys

def health_check(server_url="http://localhost:8001"):
    try:
        response = requests.get(f"{server_url}/api/configs", timeout=10)
...
def status(server_url="http://localhost:8001"):
    try:
...
cat > /opt/perf-platform/scripts/quick_test.sh << 'EOF'
#!/bin/bash
...
cat > /opt/perf-platform/configs/test_server.json << 'EOF'
{
  "name": "Test Server",
  "os_ip": "192.168.1.100",
  "os_user": "dell",
